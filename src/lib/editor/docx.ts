 ...
 * ── 왜 DOCX 도 만드는가 ────────────────────────────────────────────────────
 *
 * 공문서의 정본은 한/글이다. 이 제품이 만든 HWPX 가 한/글에서 열리는 것은
 * 실물로 확인했지만(pack.ts 머리말), **받는 자리에 한/글이 없을 수 있다.**
 * 「받는 사람이 열 수 있는 파일」이 하나뿐이면 그것만으로 위험하다. 워드는
 * 관공서에서 두 번째로 흔한
 ...
 * 한다. pack.ts 도 한/글 확인을 마치기 전까지 같은 자리에 같은 경고를 달고
 * 있었다. **그 쪽은 걷어냈고 이 쪽은 그대로다** — 확인한 것과 안 한 것을
 * 같이 적어 두면 둘 다 못 믿는다.
 *
 * ── 규격 ───────────────────────────────────────────────────────────────────
 *
