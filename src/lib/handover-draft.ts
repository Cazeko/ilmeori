 ...
 * ── 왜 표인가 ──────────────────────────────────────────────────────────────
 *
 * 이 서식의 여러 칸은 **업무 한 건이 한 문단**이다. 그런데 이어진 글줄로 놓으면
 * 스물몇 줄에서 업무 넷의 사실이 어디서 끊기는지 눈으로 찾을 수 없다 —
 * *"눈으로 읽기에 가시성이 떨어진다"*.
 *
 * 별지 제12호서식은 **표를 금지하지 않는다.** 실제로 손으로 채우는 인계서는
 * 대개 「업무 / 내용」 두 칸짜리 표로 적고, 이 서식의 사람 표·서명란도 이미
 * 표다. 바꾸는 것은 **칸 안의 배치**뿐이고 칸의 이름과 순서는 법이 정한 그대로다.
 *
 * ── 글자는 한 자도 안 옮긴다 ──────────────────────────────────────────────
 *
 ...
 * ── 왜 여기 있나 ──────────────────────────────────────────────────────────
 *
 * 이 함수는 `print-sheet.tsx` 안에 있었다. 그동안 그 파일이 서식을 그리는
 * **유일한** 자리였기 때문이다. 한/글 내보내기가 생기면서 서식을 짓는 자리가
 * 둘이 됐고, 짜임이 두 곳에 있으면 그 자리에서 갈라진다 — 화면은 표인데
 * 한/글 파일은 글줄인 인계서가 나온다. 짜임도 문단과 같은 곳에서 나온다.
 */
...
export function chunkParagraphs(paragraphs: DraftParagraph[]): DraftChunk[] {
 ...
 * 화면에서는 항목마다 근거를 붙이지만, 종이와 한/글 파일에서는 서식을
 * 어지럽히므로 맨 아래에 한 번 모아 적는다.
 *
 * ── 왜 문자열 하나인가 ────────────────────────────────────────────────────
 *
 * 이 문단은 `print-sheet.tsx` 의 JSX 안에서 조립되고 있었다. 한/글 내보내기가
 * 같은 말을 따로 적으면 **종이와 파일이 서로 다른 수를 말하는 날**이 온다.
 * 이 제품이 파는 유일한 정직성이 그 수이므로, 두 매체가 한 함수를 부른다.
 *
 * 갈래가 셋이다. 처음에는 「안 실린 것이 있나」 하나로만 갈랐는데, **들여다본
 * 것이 0건인 인계**가 그 else 로 떨어져 「0건을 들여다보고 그 전부를
 * 실었습니다 … 화면에서 확인할 수 있습니다」가 찍혔다. 그 인계에서는 화면의
 * 미포착 판이 아예 안 그려지므로(seen 0 이면 null), 종이가 **없는 판을
 * 가리키고** 있었다.
 */
export function sheetSourceText({
  evidence,
  screening,
  hasNotes,
}: {
  evidence: HandoverDraft["evidence"];
  screening: HandoverDraft["screening"];
  hasNotes: boolean;
}): string {
  const s = screeningTotal(screening);
    ...
    s.seen === 0
      ...
      : s.notUsed > 0
        ? // 「안 실린 N건은 이 종이에 없습니다」로 적었다가 고쳤다.
          // **없는 것은 본문이지 제목이 아니다** — 본문이 안 실린 문서 항목도
          // 「2. 관련 문서 현황」에는 제목이 줄줄이 찍혀 있다. 종이를 든 사람이
          // 그 제목을 보고 「저기 있잖아요」 하면, 이 문단이 방금 한 말이 그
          // 자리에서 거짓이 된다.
          `규칙은 일머리에 남은 대화·문서 항목 ${s.seen}건을 들여다보고 그중 ${s.used}건의 본문을 실었으며, 안 실린 ${s.notUsed}건의 본문은 이 문서에 없습니다. 무엇을 기준으로 골랐고 무엇이 안 실렸는지는 화면에서 항목별로 확인할 수 있습니다.`
        : `규칙은 일머리에 남은 대화·문서 항목 ${s.seen}건을 들여다보고 그 전부의 본문을 실었습니다. 무엇을 기준으로 골랐는지는 화면에서 확인할 수 있습니다.`;

  return [
    `이 초안은 「일머리」에 쌓인 기록(업무 ${evidence.works}건 · 문서 ${evidence.documents}건 · 대화 ${evidence.comments}건 · 이력 ${evidence.activities}건 · 첨부 ${evidence.attachments}건)에서 서식 순서대로 뽑아 정리한 것입니다.`,
    screened,
    "사람이 확인하고 보태야 하는 초안이며, 그대로 제출하는 문서가 아닙니다.",
    // 보탠 글이 있을 때만 적는다. 없는데 적어 두면 문서만 든 사람이 어딘가에
    // 사람이 쓴 문장이 있다고 여기고 찾게 된다.
    hasNotes
