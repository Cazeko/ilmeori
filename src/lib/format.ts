 ...
 *
 * ⚠ **이것은 문장과 종이의 표기다.** 별지 제12호서식과 인계 초안이 이 함수를
 *   쓴다(handover-draft.ts). 결재에 올라가는 종이에 「D+28」이 찍히면 안 된다 —
 *   아래 formatDueDday 와 자리를 나눠 쓰는 이유가 그것이다.
 */
...
/**
 * 같은 사실의 **좁은 자리** 표기 — `D-5` · `D+28` · `D-DAY`.
 *
 * ── 왜 두 가지를 두는가 ────────────────────────────────────────────────────
 *
 * 「28일 지남」은 여섯 글자다. 카드 머리 줄에서는 배지와 나란히 서고, 홈의
 * 「그 다음」 줄에서는 제목 옆에 붙는다. 그 자리에서 여섯 글자는 **읽는 것이
 * 아니라 훑는 것**인데, 글자가 길면 훑기가 안 되고 줄이 벌어진다.
 *
 * D-day 는 이 사용자들이 이미 쓰는 표기다. 공문 기한, 행사 준비, 감사 대비가
 * 전부 그 말로 돈다. 새 어휘를 들여오는 것이 아니라 **원래 쓰던 말로 줄이는
 * 것**이라 배울 것이 없다.
 *
 * ── 나누는 기준은 「자리」다 ───────────────────────────────────────────────
 *
 *   좁은 자리(칩·카드·목록 줄·히어로 숫자)   D+28        ← 이 함수
 *   문장·종이(상세 설명·인계 초안·별지 서식)  28일 지남   ← formatDueLabel
 *
 * 종이 쪽을 바꾸지 않는 것이 중요하다. 별지 제12호서식은 결재에 올라가고,
 * 거기 적히는 것은 훑는 글이 아니라 **읽는 글**이다.
 *
 * ── 소리로는 뜻이 없다 ────────────────────────────────────────────────────
 *
 * 「D+28」을 화면 낭독기가 읽으면 「디 플러스 이십팔」이다. 그래서 부르는
 * 자리에서 **눈에는 이 값을, 소리에는 formatDueLabel 을** 준다
 * (aria-hidden + sr-only 한 쌍). 이 저장소가 대화·첨부 수에서 이미 쓰는 방식이다.
 */
export function formatDueDday(dueDate: string, from?: string): string {
  const d = daysUntil(dueDate, from);
  // 마감 당일에 「D-0」이라고 적지 않는다 — 0 은 「없다」로도 읽힌다.
  if (d === 0) return "D-DAY";
  return d < 0 ? `D+${-d}` : `D-${d}`;
}
