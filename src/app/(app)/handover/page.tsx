                ...
                {done ? (
                  <p className="flex items-start gap-2 text-body-sm break-keep text-gray-60">
                    ...
                    <span>
                      인계가 완료되었습니다.
                      {/* ── 물어보는 자리를 화면이 말한다 ────────────────────
                          인터뷰가 말한 20~30통의 전화는 「물어볼 곳이 없어서」만이
                          아니라 **「물어볼 자격이 남아 있는지 몰라서」** 생긴다.
                          남아 있다 — 정책이 그렇게 두었고(handover_note·work_member),
                          넘긴 사람은 열람자로 남는다.

                          **그리고 그 자리는 쪽지가 아니라 업무의 대화다.**
                          이 제품에서 쪽지는 공개 범위 **밖**의 사람을 위한 것이라
                          받는 사람 목록에서 참여자를 빼 놓았고(work-notes.tsx),
                          넘긴 사람은 열람자로 **참여자에 남아** 애초에 고를 수
                          없다. 그러니 화면이 먼저 그 자리를 말해야 헤매지 않는다.

                          왜 그렇게 정했는지도 한 줄로 남긴다. 개인 쪽지로 오간
                          말은 그 두 사람에게서 끝나고, **다음 인계에서 또 스무
                          번 전화하게 만드는 것이 정확히 그 구조**다. 대화에 적으면
                          그 업무에 남고 다음 인계서가 그것을 다시 읽는다. */}
                      {!isSender ? (
                        <>
                          {" "}
                          {from.name} {from.position}
                          ...
                          에 적어 주세요. 쪽지로 주고받으면 두 사람에게서
                          끝나지만, 대화는 그 업무에 남아 다음 인계서가 다시
                          읽습니다.
                        </>
                      ) : (
                        <>
                          {" "}
                          각 업무의 이력 탭에서 권한이 옮겨 간 기록을 확인할 수
                          있습니다. {to.name} {to.position}
                          {josa(to.position ?? to.name, "이", "가")} 물어보면 그
                          업무의 「대화」에서 답할 수 있습니다 — 열람 권한이
                          남아 있습니다.
                        </>
                      )}
                    </span>
