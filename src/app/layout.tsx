...
export const metadata: Metadata = {
  title: {
    default: "일머리 | 인수인계 자동화 플랫폼",
    template: "%s · 일머리",
  },
  // 한 줄 소개. 로그인 화면·README·발표 대본과 **같은 문장**이다 —
  // 자리마다 다르게 적으면 그건 한 줄 소개가 아니라 네 개의 설명이다.
  //
  // 끝의 짧은 구절(태그라인, 위 title.default 도 같은 자리)은 그 문장에
  // 속하지 않는다 — 검색·탭 제목·공유 미리보기에 붙는 분류어다. 「기능이
  // 뭔지 모르겠다」는 1차예선 심사평을 받고 「인수인계 자동화 플랫폼」으로
  // 못박았다. 지정과제 N7의 공식 명칭(「부서 간 협업 업무공유 플랫폼」)은
  // 바꾼 게 아니라 자리를 옮겼다 — README 부제·로그인 화면 배지가 그 이름을
  // 그대로 인용한다. 여기서까지 두 이름을 다 적으면 탭 제목이 길어진다.
  description:
    "평소 결재와 대화가 쌓이면, 근거가 붙은 한/글 결재문서와 법정 인계서가 저절로 나옵니다. 화성시 인수인계 자동화 플랫폼",
