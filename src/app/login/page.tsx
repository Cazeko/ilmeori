 ...
 * 대부분 거기서 닫는다. 그래서 계정 만들기를 두지 않고, 역할이 다른 데모 계정을
 * 눌러서 바로 들어가게 했다. 수는 목업이 정한다(demoAccounts) — 여기 적어 두면
 * 계정이 하나 느는 날 화면만 옛 수를 말한다.
 *
      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-5 py-10 lg:grid-cols-[1fr_1.1fr] lg:gap-12 lg:py-20">
        ...
        <div className="lg:pt-6">
          ...
          >
            ...
            {/* ── 세 숫자가 서로를 검산한다 ─────────────────────────────
                앞의 판은 「1.14개월 · 30.2% · 35.0%」였다. 1.14와 30.2는
                **같은 사실의 두 표현**이다(1.14 ÷ 3.78 = 30.2%). 나란히 세워
                두면 손실을 두 번 센 것처럼 읽히고, 검산하는 사람 앞에서는
                그게 곧 흠이 된다.

                그래서 원수치를 앞에 놓는다. 3.78 − 2.64 = 1.14 가 화면 위에서
                맞아떨어지므로 숫자를 믿을 이유가 화면 안에 있다. */}
            <dl className="mt-4 grid grid-cols-3 gap-3">
              {[
                ["3.78개월", "인계가 부실할 때 업무 파악에 걸린 기간"],
                ["2.64개월", "충실히 인계했을 때"],
                ["1.14개월", "그 차이 — 30.2% 단축"],
              ].map(([figure, label]) => (
                <div key={label}>
            ...
            <p className="mt-4 text-body-xs leading-relaxed break-keep text-gray-60">
              ...
              인계인수를 중심으로」(한국행정연구원, 2011). 응답자 35.0%가 현행
              인계를 비효율적이라고 답했고, 대면 인계인수는 통상 30분 안에 끝나는
              것으로 나타났습니다.
            </p>
          </section>
        </div>
