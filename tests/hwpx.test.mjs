 ...
 * 컴퓨터에서 사람이 한 번 열어 봐야만 안다(계획서 §5.3). **그 확인은 끝났고,
 * 열렸다**(pack.ts 머리말). 다만 그것은 사람이 한 것이지 이 파일이 한 것이
 * 아니므로, 이 시험의 범위는 그대로다. 여기서 보는 것은
...
console.log(
  "\n※ 이 시험이 보는 것은 「규격대로 짜였는가」까지입니다. 「한/글에서 열린다」는\n" +
    "  한/글이 있는 컴퓨터에서 한 번 열어 확인했습니다(pack.ts 머리말). 인쇄(A4)\n" +
    "  폴백은 그대로 둡니다 — 그 자리에 한/글이 없을 수도 있기 때문입니다.",
);
