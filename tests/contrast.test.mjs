...
console.log("\n가장 옅은 글자 — 세 층 전부에서 읽히는가");
// ── gray-50 은 글자가 아니다 ────────────────────────────────────────────────
//
// 이 시험이 한동안 gray-50 을 **테두리로만** 재고 있었다(바로 위 줄, 3:1).
// 그 사이 gray-50 이 글자로도 네 곳에 쓰이고 있었고, 실측하면 이렇다.
//
//     흰 종이 gray-0   4.51   ← 기준선 위 0.01. 여백이 아니라 운이다
//     판     surface   4.32   ← 미달
//     바탕   gray-5    3.99   ← 미달
//
// 로그인 화면의 행정안전부 출처 문단이 바탕 위 gray-50 이었다(3.99:1, AA 미달).
// 시험이 통과한 채로 미달이 남아 있었던 것은 **재지 않는 쌍이었기 때문**이다.
//
// 그래서 글자로 쓰는 가장 옅은 색을 gray-60 으로 못박고, 세 층 전부를 잰다.
// (gray-50 은 이제 disabled: 뒤에서만 쓰인다 — tests/design-lint.test.mjs 가 지킨다)
contrast("gray-60", "gray-0", 4.5, "흰 종이 위");
contrast("gray-60", "surface", 4.5, "판 위");
contrast("gray-60", "gray-5", 4.5, "바탕 위");
...
// ── 누를 수 없는 단추 ────────────────────────────────────────────────────────
//
// 한동안 `disabled:opacity-50` 이었다. 투명도로 낮추면 **읽을 수 있는지까지**
// 함께 내려간다 — 흰 글자를 판 위에서 절반 섞으면 실측 2.78:1 이다.
//
// WCAG 1.4.3 은 비활성 조작기를 면제하지만 이 저장소는 그 면제를 쓰지 않기로
// 했다. 사용자에 장애인 공무원이 포함된다고 스스로 적어 둔 공공 시스템이고
// (globals.css 의 focus 주석), 「왜 안 눌리는지」를 읽을 수 있어야 그다음을
// 할 수 있기 때문이다.
//
// 그리고 색 언어 문제이기도 하다 — 이 앱의 파랑은 갈래가 하나다(「누를 수
...
// ── 못 고치는 입력칸도 읽혀야 한다 ────────────────────────────────────────
//
// 같은 규칙이 입력칸에도 걸린다. 한동안 안 걸려 있었던 이유는 **그 칸이 화면에
// 뜬 적이 없어서**다 — 데모(읽기 전용)에서 폼을 통째로 안 그리고 「지금은 읽기
// 전용입니다」 한 상자로 갈음했다(DESIGN.md §18.5). 폼을 감추지 않고 못 쓰게만
// 그리기로 하면서, 이 짝이 처음으로 사람 눈앞에 선다.
//
// 그전 값은 `disabled:text-gray-50` 이었고 바탕(gray-5) 위에서 4.0:1 로
// **본문 기준 4.5 에 미달**이었다. 단추에서 gray-60 을 고른 것과 같은 이유로
// 여기도 gray-60 이다 — 못 고치는 것과 못 읽는 것은 다른 말이다.
contrast("gray-60", "gray-5", 4.5, "비활성 입력칸 글자");
