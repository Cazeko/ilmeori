 ...
 * 컴퓨터에서 사람이 한 번 열어 봐야만 알고, **아직 하지 않았다.** (한/글 쪽은
 * 같은 단서가 붙어 있다가 사람이 열어 보고 걷어냈다 — hwpx.test.mjs 머리말.
 * **여기만 남았다.**) 여기서 보는 것은 그 앞 단계다 — 규격대로 된 ZIP 인가,
...
console.log(
  "\n⚠ 이 시험은 「워드에서 열린다」를 증명하지 않습니다. 그 확인은 워드가 있는\n" +
    "  컴퓨터에서 한 번 해야 하고, 아직 하지 않았습니다. (HWPX 쪽은 한/글로 열어\n" +
    "  확인을 마쳤습니다 — tests/hwpx.test.mjs)",
);
