...
const GONE = [
  ...
  ["bg-info-bg", "알림 상자의 파랑 면 — 브랜드 파랑 옆에 두 번째 파랑이었다"],
  ["text-info", "같은 이유"],
  ["bg-success-bg", "알림 상자의 초록 면 — 네 갈래 색 언어 밖이다"],
  /* 글자 회색은 셋뿐이다 — 90(본문·제목·강조) · 60(보조·메타) · 40(장식, 글자
     아님). 한동안 다섯 단을 썼는데 60 과 70, 80 과 90 은 나란히 놓아도 눈이
     가르지 못했다 — 위계는 단이 많다고 생기지 않는다. 바탕·선(bg-gray-80
     같은)으로는 여전히 쓴다. 편집기의 종이(styles/editor.css)는 내보내는
     문서와 값을 맞추는 자리라 이 규칙 밖이다. */
  ["text-gray-70", "글자 회색은 90·60·40 셋뿐이다 — 60 과 가를 수 없었다"],
  ["text-gray-80", "글자 회색은 90·60·40 셋뿐이다 — 90 과 가를 수 없었다"],
];
const revived = [];
for (const { path, code } of tsxFiles) {
  code.split("\n").forEach((line, i) => {
