...
이 문서대로 하면 DB는 준비된다. **애플리케이션 코드는 아직 목업을 읽는다.**

`src/lib/data.ts` 의 함수 본문과 `src/lib/session.ts` 를 Supabase 질의로 바꾸는 작업이 남아 있다. 화면 코드는 그대로 두면 된다 — 반환 타입을 조인 결과 모양으로 미리 고정해 뒀기 때문이다.
